      ...
      <main className="mt-24 pt-12 flex-1">
        ...
        <section className="px-6 py-12">
          <motion.div
            ...
            transition={{ delay: 0.2 }}
          >
            {['DESIGN', 'WEB DEVELOPMENT', 'UI/UX'].map((label, i) => (
              <motion.button
                key={label}
        ...
        <section className="px-6 py-12">
          <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              {
                ...
                sub: 'ALUMNI 21 PROGRAM',
                color: 'card-purple',
                icon: Folder,
                path: '/alumni-employment-assistance'
              },
              {
                title: 'LEARNING PLATFORM WALKTHROUGH',
                sub: '',
                color: 'card-gray',
                icon: GraduationCap,
            ...
            ].map(({ title, sub, color, icon: Icon, path }, index) => (
              <motion.div
                key={index}
                className={`folder-card ${color} cursor-pointer relative`}
                initial={{ opacity: 0, y: 40 }}
                animate={{ opacity: 1, y: 0 }}
              ...
              >
                <div className="mb-8">
                  <span className="text-xs uppercase tracking-wider opacity-70">{sub}</span>
                </div>
                <h3 className="text-2xl font-bold mb-4">{title}</h3>
                <Icon className="text-white/30 h-12 w-12 absolute bottom-4 right-4" />
              </motion.div>
            ))}
          </div>
        </section>
